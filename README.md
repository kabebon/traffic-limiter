...
Три ветки поведения (определяются по реальному статусу юзера в панели, а не по «маркерам» в тексте):

- подписка активна → `🟢 VPN · whitelist active`, тело отдаёт панель
...
- **подписка истекла по дате** (`status = EXPIRED`) → `🔴 Подписка истекла · продлите в кабинете`, тело = **только rescue-сервер** (`FAILOVER_CONFIG`), запрос в панель не идёт

> Ключевое отличие failover от снятия whitelist: при исчерпании **трафика** у юзера остаются рабочие basic-ноды и подписка числится активной до оплаченной даты — rescue-сервер тут **не** выдаётся. Rescue выдаётся **только** когда подписка реально истекла по дате, и нужен он лишь для того, чтобы юзер дотянулся до личного кабинета.
...
Happ/INCY ──GET──► /sub/{shortUuid}[/...]   (на traffic-limiter, не на панель)
                      ...
                      ├─ читает status из панели (кэш) и wl_state из SQLite
                      ├─ если status=EXPIRED и задан FAILOVER_CONFIG →
                      │      отдаёт только rescue-сервер + title «истекла», в панель не идёт
                      └─ иначе проксирует запрос на панель /api/sub/{short}[...],
...
1. В `.env`:
   ...
   WL_TITLE_EXPIRED=🔴 Подписка истекла · продлите в кабинете
   FAILOVER_CONFIG=vless://...@rescue.example.com:443?...#RESCUE
